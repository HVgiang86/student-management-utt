...
object LocalAccountLoader {
    ...
    fun getStoredAccountList(): MutableList<Account> {
        ...
        if (numberOfAccount != 0) {
            ...
            for (i in 0.until(numberOfAccount)) {
                ...
                val gender: Boolean =
                    if ("male".equals(other = genderString, ignoreCase = true)) Gender.MALE
                    else Gender.FEMALE

                Log.d(
                    "ACCOUNT TAG",
                    "account: username:$username; password:$password; email: $email; phone: $phoneNumber; gender: $genderString; address: $address"
                )
    ...
    fun storeAccountList(accountList: MutableList<Account>) {
        ...
        for (i in 0.until(accountList.size)) {
            ...
            if (account.gender == Gender.MALE) jsonObject.put(GENDER_KEY, "male")
